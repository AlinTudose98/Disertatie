package com.alint.disertatie.engine.javadssengine;

import com.alint.disertatie.engine.javadssengine.dss.STIDSSValidator;
import eu.europa.esig.dss.simplecertificatereport.SimpleCertificateReport;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.context.ApplicationContext;

import java.text.ParseException;

@SpringBootTest
class JavaDssEngineApplicationTests {
    private final ApplicationContext context;

    @Autowired
    public JavaDssEngineApplicationTests(ApplicationContext context) {
        this.context = context;
    }

    @Test
    void contextLoads() {
    }

    @Test
    void validateCertificate() throws ParseException {
        String base64Certificate = "";

        STIDSSValidator validator = context.getBean("DSSValidator", STIDSSValidator.class);

        SimpleCertificateReport report = validator.validateCertificate(base64Certificate,"06-05-2023T10:00");

        System.out.println(report.toString());
    }
}
