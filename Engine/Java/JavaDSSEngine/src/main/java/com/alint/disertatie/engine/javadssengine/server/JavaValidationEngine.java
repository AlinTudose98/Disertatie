...
import com.alint.disertatie.engine.javadssengine.message.CertificateValidationResponse;
import com.alint.disertatie.engine.javadssengine.message.SignedFileValidationResponse;
import com.alint.disertatie.engine.javadssengine.util.JavaEngineTransceiver;
...
public class JavaValidationEngine {
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                JavaEngineTransceiver transceiver = new JavaEngineTransceiver(clientSocket.getInputStream(), clientSocket.getOutputStream());
        ...
        private void validateCertificate(JavaEngineTransceiver transceiver) throws IOException {
            ...
            for (XmlChainItem certificate : report.getJaxbModel().getChain()) {
                ...
                result.setQualificationAtValidation(new CertificateQualification());
                ...
                result.setQualificationAtIssuance(new CertificateQualification());
                ...
                result.setRevocationStatus(new RevocationStatus());
        ...
        private void validateSignature(JavaEngineTransceiver transceiver) throws IOException {
            byte[] signatureFile = transceiver.readBytes();

            SimpleReport report = validator.validateSignature(signatureFile);

            SignedFileValidationResponse response = new SignedFileValidationResponse();

            SignedFileValidationResult result = new SignedFileValidationResult();

            result.setValidationPolicy(new ValidationPolicy(report.getJaxbModel().getValidationPolicy().getPolicyName(),report.getJaxbModel().getValidationPolicy().getPolicyDescription()));
            ...
            for(XmlToken signature : report.getJaxbModel().getSignatureOrTimestamp()) {
                ...
                SignatureValidationResult signatureResult = new SignatureValidationResult();
                ...
                warnings.forEach(w -> signatureResult.getWarnings().add(new CustomMessage(w.getKey(), w.getValue())));
